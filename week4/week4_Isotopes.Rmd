...
# session info
starting_delta_values = seq(-10, 30, 10) #Starting delta values for the resevoir
...
for (i in 1:length(starting_delta_values)) { #Iteratively adding columns for each of the starting values for all the remaining water fractions. USED FOR QUESTION 3 BELOW
  ...
  mutate(!!sym(toString(starting_delta_values[i])) :=  remaining_Delta(starting_delta_values[i], remaining_Water_Fraction, alpha)) #Building the data with the function for remaining isotope value
}
...
produce_Plot <- function(...) { #Making a plot as a function. Takes y axis as input
  titley <- paste("Starting delta of 18O: ", toString(starting_delta_values[i]), " permil") #Building the plot title
  plot <- ggplot(calculated_Data) + 
  geom_point(aes(x = remaining_Water_Fraction, ...))+ #Points but we take the unknown input and stuff it into the y axis
  labs(y = titley , x = "Remaining Water Fraction")
  
  return(plot)
  }

i <- 1 #Iteratively making a plot for each starting value using the above function
